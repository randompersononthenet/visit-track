      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <span className="text-sm font-medium w-32">Visitors:</span>
            <button
            ...
            >
              {downloading === '/api/reports/visitors.csv' ? 'Downloading CSV...' : 'CSV'}
            </button>
            <button
              className="inline-flex items-center gap-2 rounded border border-slate-300 bg-white hover:bg-slate-50 px-3 py-1.5 text-sm text-slate-900 dark:bg-slate-900 dark:border-slate-700 dark:hover:bg-slate-800 dark:text-slate-100"
              onClick={() => {
                const qs = dateQuery ? `?${dateQuery}` : '';
                downloadPdf(`/api/reports/visitors.pdf${qs}`, 'visitors.pdf');
              }}
              disabled={downloading !== null}
            >
              {downloading === '/api/reports/visitors.pdf' ? 'Downloading PDF...' : 'PDF'}
            </button>
          </div>

          {/* Personnel */}
          <div className="flex flex-wrap gap-2 items-center">
            <span className="text-sm font-medium w-32">Personnel:</span>
            <button
              className="inline-flex items-center gap-2 rounded border border-slate-300 bg-white hover:bg-slate-50 px-3 py-1.5 text-sm text-slate-900 dark:bg-slate-900 dark:border-slate-700 dark:hover:bg-slate-800 dark:text-slate-100"
              onClick={() => {
                const qs = dateQuery ? `?${dateQuery}` : '';
              ...
              disabled={downloading !== null}
            >
              {downloading === '/api/reports/personnel.csv' ? 'Downloading CSV...' : 'CSV'}
            </button>
            <button
              className="inline-flex items-center gap-2 rounded border border-slate-300 bg-white hover:bg-slate-50 px-3 py-1.5 text-sm text-slate-900 dark:bg-slate-900 dark:border-slate-700 dark:hover:bg-slate-800 dark:text-slate-100"
              onClick={() => {
                const qs = dateQuery ? `?${dateQuery}` : '';
                downloadPdf(`/api/reports/personnel.pdf${qs}`, 'personnel.pdf');
              }}
              disabled={downloading !== null}
            >
              {downloading === '/api/reports/personnel.pdf' ? 'Downloading PDF...' : 'PDF'}
            </button>
          </div>

          {/* Visit Logs */}
          <div className="flex flex-wrap gap-2 items-center">
            <span className="text-sm font-medium w-32">Visit Logs:</span>
            <button
              className="inline-flex items-center gap-2 rounded border border-slate-300 bg-white hover:bg-slate-50 px-3 py-1.5 text-sm text-slate-900 dark:bg-slate-900 dark:border-slate-700 dark:hover:bg-slate-800 dark:text-slate-100"
              onClick={() => {
                const params = new URLSearchParams();
              ...
              disabled={downloading !== null}
            >
              {downloading === '/api/reports/visit-logs.csv' ? 'Downloading CSV...' : 'CSV'}
            </button>
            <button
              className="inline-flex items-center gap-2 rounded border border-slate-300 bg-white hover:bg-slate-50 px-3 py-1.5 text-sm text-slate-900 dark:bg-slate-900 dark:border-slate-700 dark:hover:bg-slate-800 dark:text-slate-100"
              onClick={() => {
                ...
                if (subjectId.trim() !== '') params.set('subjectId', subjectId.trim());
                const qs = params.toString();
                downloadPdf(`/api/reports/visit-logs.pdf${qs ? `?${qs}` : ''}`, 'visit-logs.pdf');
              }}
              disabled={downloading !== null}
            >
              {downloading === '/api/reports/visit-logs.pdf' ? 'Downloading PDF...' : 'PDF'}
            </button>
          </div>
        </div>
        <div className="flex items-center gap-4 text-sm text-slate-700 dark:text-slate-300 mt-1 flex-wrap">
          <label className="text-slate-600 dark:text-slate-400">Rows per file (CSV only):</label>
          ...
          />
          <span className="text-xs text-slate-500">Filters apply to both. Paging applies to CSV. PDF is capped at 500 rows.</span>
        </div>
      </div>
    </div>
  );
